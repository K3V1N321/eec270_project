...
uint64_t PermuteAddress(uint64_t addr) {
    return (addr ^ 0x5A3C1B7E);  // XOR-based permutation for better distribution
}

Address Config::AddressMapping(uint64_t hex_addr) const {
	
	hex_addr >>= shift_bits;
    uint64_t permuted_addr = PermuteAddress(hex_addr);

    int channel = (permuted_addr >> ch_pos) & ch_mask;
    int rank = (permuted_addr >> ra_pos) & ra_mask;
    int bg = (permuted_addr >> bg_pos) & bg_mask;
    int ba = (permuted_addr >> ba_pos) & ba_mask;
    int ro = (permuted_addr >> ro_pos) & ro_mask;
    int co = (permuted_addr >> co_pos) & co_mask;
