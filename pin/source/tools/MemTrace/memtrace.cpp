...
{
    public:
        TRACEINFO(ADDRINT address, ADDRINT time, std::string type) {
            _address = address;
            _time = time;
            _type = type;
        }
    ...
    for (int cmd = 0; cmd < traceHeader->NumCmds(); cmd++)
    {
        switch (traceHeader->CmdType(cmd))
        {
            case TCMD_IMMEDIATE:
                ...
                {
                    // ofile << std::showbase << std::hex << Immediate(logCursor) << " "
                    // << traceHeader->Ref(ref)->GetAccessType() << " "
                    // << std::dec << clockCycle << endl;
